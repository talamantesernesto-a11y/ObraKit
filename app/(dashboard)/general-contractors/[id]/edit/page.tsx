...
export default function EditContractorPage() {
  ...
  const gcId = params.id as string
  const { toast } = useToast()
  const [loading, setLoading] = useState(false)
  const [ready, setReady] = useState(false)

  const { register, handleSubmit, formState: { errors }, reset } = useForm<ContractorFormData>({
  ...
  const onSubmit = async (data: ContractorFormData) => {
    setLoading(true)
    try {
      await updateContractor(gcId, data as Record<string, unknown>)
